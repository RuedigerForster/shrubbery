...
def _t_exclude_reduces_kappa_error():
    ...
    err_all  = abs(float(np.mean(m_all.kappa["pressure"]))  - TRUE_KAPPA)
    err_excl = abs(float(np.mean(m_excl.kappa["pressure"])) - TRUE_KAPPA)
    assert err_excl < err_all, (
        f"exclude= did not reduce κ error: all={err_all:.2e} excl={err_excl:.2e}"
    )
    assert np.all(np.sign(m_excl.kappa["pressure"]) == np.sign(TRUE_KAPPA)), \
        f"κ wrong sign after exclude: {np.round(m_excl.kappa['pressure'], 6)}"
...
def _t_exclusion_fires_one_warning():
    """The exclusion warning fires exactly once from the fit() call."""
