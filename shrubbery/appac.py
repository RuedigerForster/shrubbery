...
NumPy implementation with full (analytic) covariance propagation.
...
Pipeline
--------
The correction separates the three contributions that share the
variance-covariance matrix: pressure-driven measurement *artifacts* (correlated
across peaks within a sample), instrument *drift and episode bias* (correlated
sample-to-sample via the instrument), and uncorrelated Gaussian *noise*.

The artifact is removed by a direct regression on the *measured* covariate — a
single COMMON κ (κ is a detector property; an FID is carbon-equimolar, so it is
not component-specific).  This is the supervised replacement for the unsupervised
PCA(1)/PC1 extraction used previously.  Because the covariate is measured, the
correction transfers to any sample that carries it, with no z-score
normalisation.  PCA is retained only for the drift (daily-factor) stage, which
runs on the κ-corrected data.  Stages are grouped by the function that runs them.

fit() — artifact correction, per-sample bias, cross-sample daily factor
    1. Estimate one common sensitivity κ (WLS on binned, ep0-referenced log-area
       deviations pooled over peaks and cylinders — never pool closures; see
       _estimate_kappa).  No PCA, no z-score: pressure is the regressor.
    2. Covariate-correct the raw areas:  Y / (1 + Σ_cv κ·(cv − ref)).
    3. Centers = GEOMETRIC mean exp(mean(log Y)) of the κ-corrected data — the
       unbiased "true value" for log-normal data (no separate χ² center-refinement
       step needed).  Fit dispersion (sd ~ center + center²) and z-score-normalise
       — preparation for the daily-factor PCA.
    4. PCA pass on the κ-corrected data → correlated component (PC1).
    5. Per-sample episode bias (geometric mean per episode).
    6. Cross-sample correlated daily factor (PC1 across concurrent cylinders).

fit_uncorrelated_drift() — optional, adds the PC2 term to the multiplier
    7. PC2 slow-drift model (common component + per-sample polynomial trend).

build_multiplier() + correct()
    8. Assemble the multiplier (below) and divide:  corrected = raw / multiplier.

propagate_covariance()
    9. Propagate input + parameter uncertainty (GUM Eq. 13; see below).
...
kappa is COMMON across peaks — one scalar per covariate, broadcast uniformly to
every peak at each apply site (κ is a detector property, not component-specific).
See _estimate_kappa.
...
Sensitivity vectors in the flattened (n_obs × n_peaks) index space:
    ...
    s_κ[t,j]        = −Y[t,j] · Δcv[t] / M[t,j]²   (one shared κ scales all peaks)
...
class AppacModel:
    ...
    kappa: dict[str, float]                  # per-covariate common sensitivity (scalar; κ is not component-specific)
    kappa_rsq: dict[str, float]              # per-covariate R² of kappa estimation (pooled)
    ...
    kappa_var: dict[str, float] = field(default_factory=dict)   # variance of the common (scalar) kappa
    kappa_resid: dict[str, np.ndarray] = field(default_factory=dict)  # WLS weighted residuals over the pooled bins
    kappa_bins: dict[str, tuple] = field(default_factory=dict)  # (dp_b, w_b, y_b, y_pred) pooled over peaks (diagnostic)
...
def _wls(X: np.ndarray, y: np.ndarray, w: np.ndarray | None = None):
    """
    (Weighted) ordinary least squares.  Minimises Σ wᵢ (yᵢ − Xᵢ·c)²;
    with ``w=None`` this is plain OLS.

    Returns
    -------
    coeffs   : (p,)    fitted coefficients
    wssr     : float   weighted sum of squared residuals  Σ wᵢ rᵢ²
    coef_var : (p,)    variance per coefficient  σ²·diag((XᵀWX)⁻¹), σ² = wssr/(n−p)
    """
    if w is None:
        Xw, yw = X, y
    else:
    ...
    res = yw - Xw @ c
    wssr = float(res @ res)
    sigma2 = wssr / max(len(y) - X.shape[1], 1)
    coef_var = sigma2 * np.diag(np.linalg.pinv(Xw.T @ Xw))
    return c, wssr, coef_var


# ──────────────────────────────────────────────────────────────────────────────
# Step 3 — common kappa estimation (WLS on binned, ep0-referenced area deviations, pooled over peaks)
# ──────────────────────────────────────────────────────────────────────────────
...
def _estimate_kappa(
    ...
    """
    Estimate a single COMMON sensitivity kappa via WLS on binned, ep0-referenced
    log-area deviations pooled across both peaks and cylinders.

    Forward model (exact for multiplicative / log-normal data):
        log(Y_j) − log(ep0_j)  ≈  κ · (covariate − ref)     for every peak j

    κ is COMMON across peaks by design — fewest parameters, and physically κ is a
    detector property (an FID is carbon-equimolar, so κ cannot be component-
    specific).  The reduced apparent sensitivity of the single largest peak (e.g.
    CH4) is variance curvature that the dispersion normalisation absorbs upstream,
    not a per-peak κ; pooling all peaks averages it out rather than over-fitting.

    Two pooling guards:
      - **never pool closures.**  Reference each cylinder to its OWN ep0 (episode-0
        geometric mean) before pooling, so different-composition cylinders share a
        common origin.  The ep0 offset is additive in log space → slope unbiased.
      - the correction is unity at covariate = ref, so it reproduces the expected
        peak area at (reference value, day 1 / ep0).

    Caveat — omitted-variable bias
    ------------------------------
    κ is regressed on the RAW ep0-referenced areas, whose deviations still carry the
    daily factor, episode drift and noise.  The slope is unbiased ONLY if every
    non-pressure driver is uncorrelated with the covariate.  Atmospheric pressure is
    seasonal, and other detector drivers (lab temperature, humidity, column ageing)
    can co-vary with it; any such confounder is silently absorbed into κ.  Because κ
    is the parameter that transfers to unknown samples, this bias travels with it.
    Pressure-binning mitigates but does not remove a confounder that tracks pressure.
    Verify by checking the corrected residual against candidate covariates (the
    covariate-screening step) before trusting κ as a transferable constant.

    Parameters
    ----------
    Y_cor_list       : per cylinder, the correlated (PC1) areas in original scale.
    covariate_lists  : {name: [array_per_sample]}  values already referenced
                       (covariate − ref), so mean ≈ 0.
    dates_list       : per cylinder, integer day numbers (for episode 0).
    breakpoints_list : per cylinder, sorted breakpoint dates; episode 0 is the
                       span before the first breakpoint.

    Returns
    -------
    kappa, kappa_rsq, kappa_var, kappa_resid, kappa_bins — dicts keyed by covariate.
    kappa / kappa_var / kappa_rsq : scalars (κ is common across peaks — one
    parameter per covariate, broadcast at each apply site).
    kappa_resid / kappa_bins : pooled diagnostics.
    """
    # ── ep0-referenced log-area deviation per cylinder (revert to areas) ───────
    # log(Y) − log(ep0); ep0 = geometric mean over episode 0 (pre-first-bp).
    # Y > 0 is guaranteed: check_inputs() raises on any non-positive area before
    # fit() reaches this point ("log(Y) is undefined").
    log_dev_list: list[np.ndarray] = []
    for Y, dates, bps in zip(Y_cor_list, dates_list, breakpoints_list):
        logY = np.log(Y)
        ep0_mask = (dates < bps[0]) if len(bps) > 0 else np.ones(len(dates), bool)
        if ep0_mask.sum() < 2:                 # too few -> fall back to whole series
            ep0_mask = np.ones(len(dates), bool)
        ep0 = logY[ep0_mask].mean(axis=0)      # (n_peaks,) log-geomean baseline
        log_dev_list.append(logY - ep0)
    ...
    # POOL ALL PEAKS into a single regression → one common κ.  Fewest parameters
    # is the better model: κ is a detector property, and an FID is carbon-equimolar,
    # so it cannot be component-specific.  The lone large peak's (e.g. CH4) reduced
    # sensitivity is curvature absorbed upstream by the dispersion normalisation,
    # not a per-peak κ.  Each peak's ep0 offset is additive in log space, so pooling
    # deviations keeps the slope clean.  y_all is covariate-independent → built once.
    y_all = np.concatenate([d.ravel() for d in log_dev_list])                     # all peaks

    for cv_name, cv_per_sample in covariate_lists.items():
        # Repeat each observation's covariate value once per peak, using THIS
        # sample's peak count (samples may have different peak sets) so dp_all
        # lines up element-for-element with y_all's per-sample ravel.
        dp_all = np.concatenate([
            np.repeat(dp_i, Yk.shape[1])
            for dp_i, Yk in zip(cv_per_sample, Y_cor_list)
        ])

        # Binned WLS by covariate (0.1-unit bins; weights = bin counts).
        # Grouped reduction in one O(N) pass (np.bincount) — not a per-bin mask loop.
        bin_keys  = np.round(dp_all, 1)
        dp_b, inv = np.unique(bin_keys, return_inverse=True)   # bin labels + index per point
        w_b = np.bincount(inv).astype(float)                   # count per bin
        y_b = np.bincount(inv, weights=y_all) / w_b            # mean of y per bin

        y_w      = float(w_b @ y_b) / w_b.sum()
        ss_tot_w = float(w_b @ (y_b - y_w) ** 2)
        dp_w     = float(w_b @ dp_b) / w_b.sum()
        ss_dp    = float(w_b @ (dp_b - dp_w) ** 2)
        if ss_dp > 0:
            X = np.column_stack([np.ones(len(dp_b)), dp_b])
            coeffs, wssr, coef_var = _wls(X, y_b, w_b)   # [intercept, slope]
            k, var, y_pred = float(coeffs[1]), float(coef_var[1]), X @ coeffs
        else:                                            # constant covariate → κ unidentifiable
            k, var, wssr, y_pred = 0.0, np.inf, ss_tot_w, np.full(len(y_b), y_w)

        # κ is COMMON across peaks — a single scalar parameter per covariate.
        kappa[cv_name]       = k
        rsq[cv_name]         = 1.0 - wssr / ss_tot_w if ss_tot_w > 0 else 0.0
        kappa_var[cv_name]   = var
        kappa_resid[cv_name] = np.sqrt(w_b) * (y_b - y_pred)
...
def _covariate_correct(
    ...
    """
    Divide raw areas by the summed covariate correction factor (common scalar κ,
    applied uniformly to every peak).

    Y_corrected[t,j] = Y[t,j] / (1 + Σ_cv  kappa[cv] × (covariate[cv][t] − ref[cv]))

    Parameters
    ----------
    Y              : (n_obs, n_peaks)  raw peak areas
    covariates     : {name: (n_obs,)}  covariate arrays (unreferenced)
    kappa          : {name: float}     common sensitivity per covariate
    ...
    """
    denom = np.ones(len(Y))                       # (n_obs,) — κ is common across peaks
    for cv, cov_array in covariates.items():
        denom = denom + float(kappa[cv]) * (cov_array - covariate_refs[cv])
    return Y / denom[:, None]
...
def fit(
    ...
    ct_init         : dict[str, ndarray], optional
        Pre-specified center values per sample.  Default (None) uses the
        geometric mean of the κ-corrected areas (unbiased for log-normal data).
    ...
    kappa_fixed     : dict[str, float], optional
        Pre-computed common κ per covariate (e.g. from a prior no-breakpoint
        fit).  A legacy per-peak array is collapsed to its mean.  When provided,
        ``_estimate_kappa`` is skipped entirely and the
        supplied value is used for covariate correction.  ``kappa_var`` is set to
        0.0 so downstream covariance propagation treats κ as a known constant.
    ...
    -------
    AppacModel
        Fitted model parameters (centers are geometric means — unbiased for
        log-normal data).  The standard pipeline continues with
        ``fit_uncorrelated_drift()``, then ``build_multiplier()`` and
    ...
    # ── Kappa estimation (artifact correction) ────────────────────────────────
    # The artifact is corrected directly by a common-κ regression on the measured
    # covariate — this replaces the old unsupervised first PCA pass (PC1), which
    # is no longer computed.  PCA survives only for the daily-factor stage below,
    # which operates on the κ-corrected data.
    ...
    if kappa_fixed is not None:
        # κ is common (one scalar per covariate); collapse a legacy array to its mean.
        kappa       = {cv: float(np.mean(np.asarray(v, float))) for cv, v in kappa_fixed.items()}
        ...
        # Estimate ONE common κ on the RAW ep0-referenced areas, pooled over all
        # peaks: drift/noise are not pressure-correlated, so pressure-binning
        # leaves the slope unbiased.  The lone large peak's (CH4) reduced
        # sensitivity is curvature absorbed downstream by the daily-factor
        # (dispersion-normalised) stage, so it is not modelled as a per-peak κ.
        kappa, kappa_rsq, kappa_var, kappa_resid, kappa_bins = _estimate_kappa(
            [Y for Y, ok in zip(Y_list, stable_mask) if ok],
            {cv: [v for v, ok in zip(vals, stable_mask) if ok]
             for cv, vals in covariate_lists_ref.items()},
            [d for d, ok in zip(dates_list, stable_mask) if ok],
            [breakpoints[s.name] for s, ok in zip(samples, stable_mask) if ok],
        )
    ...
    # ── Dispersion + z-score normalisation of the κ-corrected data ────────────
    # Feeds the daily-factor PCA below (the only surviving PCA stage).
    # Centers are the GEOMETRIC mean exp(mean(log Y)) — the unbiased "true value"
    # for multiplicative/log-normal data (the arithmetic mean sits exp(σ²/2) high).
    # ct_init optionally pins the centers (used by callers that fix them).
    if ct_init is not None:
        ct2_list = [ct_init[s.name] for s in samples]
    else:
        ct2_list = [np.exp(np.log(Y).mean(axis=0)) for Y in Y_corrected]
...
def build_multiplier(sample: SampleData, model: AppacModel) -> np.ndarray:
    ...
    # Environmental covariate contributions — common scalar κ, uniform across peaks
    cov_sum = np.zeros(len(dates))                               # (n_obs,)
    for cv, kap in model.kappa.items():
        cov_sum = cov_sum + float(kap) * (sample.covariates[cv] - model.covariate_refs[cv])
...
def correct(Y: np.ndarray, multiplier: np.ndarray) -> np.ndarray:
    """
    Apply APPAC correction:  corrected = raw / multiplier.
...
def propagate_covariance(
    ...
    Five independent uncertainty sources are summed analytically — no AD:
    measurement, (shared) κ, episode bias, between-sample daily_corr, and PC2
    polynomial coefficients (see the module docstring for the GUM Eq. 13 terms).
    ...
    # ── κ uncertainty — one SHARED κ per covariate → a single rank-1 outer
    # product over ALL (obs, peak).  The same κ error scales every peak together,
    # so its contribution is fully correlated across peaks (not per-peak blocks).
    # s_κ[t,j] = ∂Y_corr[t,j]/∂κ = −Y[t,j]·ΔP[t] / M[t,j]²
    for cv, kap_var in model.kappa_var.items():
        var = float(kap_var)                      # common κ: one shared scalar variance
        if not (np.isfinite(var) and var > 0):    # skip NaN and +inf (unidentifiable κ → no term)
            continue
        dp = sample.covariates[cv] - model.covariate_refs[cv]   # (n_obs,)
        s_kappa = -(Y * dp[:, None]) / M ** 2                   # (n_obs, n_peaks)
        s_flat  = s_kappa.ravel()
        cov_out += var * np.outer(s_flat, s_flat)
...
def fit_generalized_correction(
    ...
    This function implements steps 2–4 of the four-step pipeline:
      1. (caller) Unbiased κ — a no-breakpoint fit (κ is ep0-referenced).
      2. Pressure correction — divide by 1 + κ·ΔP (common κ) using
         kappa_fixed (or model.kappa if kappa_fixed is not supplied).  κ is the
         only parameter that generalises to unknown samples.
    ...
    kappa_fixed   : unbiased κ from the no-breakpoint fit.
...
def build_multiplier_unknown(
    ...
    The unknown sample's composition need not be known.  The correction comprises:
      - κ covariate correction (pressure) — one common κ applied to every peak
      - PC1 correlated daily factor — uniform across peaks
      - PC2 drift/bias — uniform across peaks
    ...
    # Environmental covariate contributions — common scalar κ, uniform across peaks.
    # (κ is composition-independent, so it applies to any peak set without alignment.)
    n_peaks = unknown.Y.shape[1]
    cov_sum = np.zeros(len(unknown.dates))
    for cv, kap in model.kappa.items():
        if cv not in unknown.covariates:
            continue
        cov_sum = cov_sum + float(kap) * (unknown.covariates[cv] - model.covariate_refs[cv])
