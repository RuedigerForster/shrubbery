...
from shrubbery.appac import (SampleData, fit, build_multiplier, correct,
                                  propagate_covariance, chi_square_fit, flag_outliers,
...
kappa_est = model.kappa["pressure"]
print(f"\n  κ  true={TRUE_KAPPA:.2e}  estimated={kappa_est:.2e}  hPa⁻¹")
print(f"  κ R²: {model.kappa_rsq['pressure']:.4f}")
print(f"  κ σ:  {model.kappa_var['pressure']**0.5:.2e}  hPa⁻¹")
...
fig.suptitle(
    f"κ fit — binned means vs ΔP  (κ={model.kappa['pressure']:.2e} hPa⁻¹,"
...
#   1. The function runs and returns a fully consistent model (no stale fields).
#   2. Centers shift toward the geometric mean (ct_chi < ct_arith for log-normal).
#   3. Corrected residuals become more symmetric (|skewness| decreases).

print("\nchi_square_fit: refining center values...")
model_chi = chi_square_fit(samples, model,
                           covariate_refs={"pressure": P_REF})

# Check 1 — all model fields are present and self-consistent (final fit was done)
for s in samples:
    assert s.name in model_chi.centers,      f"centers missing for {s.name}"
    assert s.name in model_chi.episode_bias, f"episode_bias missing for {s.name}"
assert "pressure" in model_chi.kappa_var, "kappa_var missing after chi_square_fit"
assert "pressure" in model_chi.kappa,     "kappa missing after chi_square_fit"

# Check 2 — χ² at the refined centers is ≤ χ² at the initial centers.
# This must hold by construction: chi_square_fit finds the minimum.
def total_chisq(mdl):
    total = 0.0
    for s in samples:
        M   = build_multiplier(s, mdl)
        Y_c = correct(s.Y, M)
        ct  = mdl.centers[s.name]
        total += float(np.sum((Y_c - ct) ** 2))
    return total

chisq_before = total_chisq(model)
chisq_after  = total_chisq(model_chi)
print(f"  χ²: before={chisq_before:.4e}  after={chisq_after:.4e}")
assert chisq_after <= chisq_before * 1.001, \
    f"chi_square_fit increased χ²: {chisq_before:.4e} → {chisq_after:.4e}"

# Check 3 — end-to-end: corrected residuals are no worse after chi_square_fit,
# and the RSD either stays the same or improves for every sample.
# (The episode bias absorbs mean offsets for smooth log-normal data, so the
# chi_square_fit effect is small on synthetic data; it matters for real data
# where outliers and integration artefacts cause asymmetry the bias cannot
# absorb.  We test that it at least does no harm.)
for s in samples:
    M_before = build_multiplier(s, model)
    M_after  = build_multiplier(s, model_chi)
    Y_c_b    = correct(s.Y, M_before)
    Y_c_a    = correct(s.Y, M_after)
    rsd_b    = (Y_c_b.std(axis=0) / Y_c_b.mean(axis=0)).mean()
    rsd_a    = (Y_c_a.std(axis=0) / Y_c_a.mean(axis=0)).mean()
    print(f"  {s.name}: RSD before={rsd_b*100:.4f}%  after={rsd_a*100:.4f}%")
    assert rsd_a <= rsd_b * 1.01, \
        f"{s.name}: chi_square_fit worsened RSD: {rsd_b*100:.4f}% → {rsd_a*100:.4f}%"

# Check 4 — κ and correction quality remain valid after chi_square_fit
kappa_chi = model_chi.kappa["pressure"]
print(f"  κ after chi_square_fit: {kappa_chi:.2e}  (was {kappa_est:.2e})")
assert np.sign(kappa_chi) == np.sign(TRUE_KAPPA), "κ sign flipped after chi_square_fit"
assert model_chi.kappa_rsq["pressure"] > 0.3, \
    f"κ R² degraded after chi_square_fit: {model_chi.kappa_rsq['pressure']:.4f}"
...
model_drift = fit_uncorrelated_drift(samples, model_chi)
...
for s in samples:
    M_nd = build_multiplier(s, model_chi)
...
masks_clean = flag_outliers(samples, model_chi)
gen_model = fit_generalized_correction(samples, model_chi,
...
M_unk = build_multiplier_unknown(unknown, gen_model, model_chi, instrument="S1")
...
Y_unk_kap  = _covariate_correct(Y_unk, {"pressure": unk_pressure},
                                  model_chi.kappa, model_chi.covariate_refs)
...
updater = GeneralizedUpdater.from_generalized_model(
    gen_model, model_chi, samples, instrument="S1"
)
